# Developed on January 23, 29 and 30, 2020.
...
class Preprocessor:
    ...
    def __init__(self, nltk_stop_words_package='portuguese'):
        urls = [
            'https://cis.uni-muenchen.de/~schmid/tools/TreeTagger/data/tree-tagger-linux-3.2.2.tar.gz',
            'https://cis.uni-muenchen.de/~schmid/tools/TreeTagger/data/tagger-scripts.tar.gz',
            'https://cis.uni-muenchen.de/~schmid/tools/TreeTagger/data/install-tagger.sh',
            'https://cis.uni-muenchen.de/~schmid/tools/TreeTagger/data/portuguese.par.gz'
            #'https://cis.uni-muenchen.de/~schmid/tools/TreeTagger/data/portuguese-finegrained.par.gz'
            #'https://cis.uni-muenchen.de/~schmid/tools/TreeTagger/data/portuguese2.par.gz'
        ]
        os.makedirs('TreeTagger', exist_ok=True)
        os.chdir('TreeTagger')
        if not all([os.path.exists(url.split('/')[-1]) for url in urls]):
            for i, url in enumerate(urls, start=1):
                download_file(url, desc='Downloading TreeTagger file %s/%s' % (i, len(urls)))
        out = subprocess.check_output(['sh', 'install-tagger.sh'])
        print(out.decode())
        os.chdir('..')
    ...
    def preprocess(self, corpus):
        print('The following progress bar will be frozen until completion.')
        tq = tqdm(desc='Preprocessing with TreeTagger', total=len(corpus), unit='doc')
        corpus = Preprocessor._preprocess_with_TreeTagger(corpus)
        tq.update(len(corpus))
        tq.close()
    ...
    @staticmethod
    def _preprocess_with_TreeTagger(corpus):
        assert all([Preprocessor.SPLIT_TEXT_TOKEN not in text for text in corpus])
        corpus = '\n{0}\n'.format(Preprocessor.SPLIT_TEXT_TOKEN).join(corpus)
        with NamedTemporaryFile(mode='w', dir='.') as stdout:
            with subprocess.Popen(['TreeTagger/cmd/tree-tagger-portuguese'], bufsize=-1, stdin=subprocess.PIPE, stdout=stdout, stderr=subprocess.PIPE) as p:
                _out, _err = p.communicate(corpus.encode())
            with open(stdout.name, 'r') as stream:
                return Preprocessor._parse_TreeTagger_output(stream)

    @staticmethod
    def _parse_TreeTagger_output(stream):
        corpus = []
        lemmas = []
        while True:
            ...
            if len(line_parts) == 3:
                token, _pos, lemma = line_parts
                if token != Preprocessor.SPLIT_TEXT_TOKEN:
                    if lemma != '<unknown>\n':
                        lemmas.append(lemma.replace('\n', ''))
                    else:
                ...
                if lemmas[-1] == 'binary file (standard input) matches':
                    lemmas = lemmas[0:-1]
                corpus.append(lemmas)
                break
            else:
