...
class Preprocessor:
    ...
    def preprocess(self, corpus):
        corpus = [list(map(lambda t: t.text, Sentence(text, use_tokenizer=True).tokens)) for text in tqdm(corpus, desc='Tokenizing', unit='doc')]
        corpus = [[t.lower() for t in token_list] for token_list in tqdm(corpus, desc='Converting to lowercase', unit='doc')]
        corpus = [[self.lemmatizer.lemmatize(t) for t in token_list] for token_list in tqdm(corpus, desc='Lemmatizing', unit='doc')]
