...
print('GPU: %s' % (spacy.prefer_gpu()))
PICKLE_PROTOCOL = 4
RANDOM_STATE = 42

class Preprocessor:
    def __init__(self, model='pt_core_news_sm', model_version='2.2.5', nltk_stop_words_package='portuguese'):
        check_call(['python', '-m', 'spacy', 'download', '-d', '-'.join([model, model_version])])
    ...
    def preprocess(self, corpus):
        ...
        for text in tqdm(corpus, desc='Preprocessing', unit='doc'):
            doc = self.nlp(text)
            tokens = [t.lemma_.lower() for t in doc if not t.lemma_ in punctuation and not t.lemma_ in self.stop_words]
