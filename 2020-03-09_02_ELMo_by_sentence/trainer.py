...
class Preprocessor:
    def __init__(self, language='portuguese', nltk_stop_words_package='portuguese'):
    ...
    def preprocess(self, corpus):
        ...
        for text in tqdm(corpus, desc='Preprocessing', unit='doc'):
            ...
            sentences = [nltk.word_tokenize(sent, self.language) for sent in sentences]
            ...
            sentences = [[t.lower() for t in token_list] for token_list in sentences]
            sentences = [[t for t in token_list if not t in punctuation] for token_list in sentences]
            sentences = [[t for t in token_list if not t in self.stop_words] for token_list in sentences]
        ...
        filenames = {
            ...
            'Tokens by sentence (NLTK)': tokens_by_sent,
            'Tokens by document (NLTK)': tokens_by_doc,
        }
...
class FeatureExtractor:
    def __init__(self):
        ...
        self.model = ElmoEmbedder(options_file=options_file, weight_file=weight_file, cuda_device=-1) # Character-based embeddings
    ...
    def extract_features(self, corpus, output_config_file='embeddings.pkl', output_embeddings_file='embeddings.dat'):
        ...
        for i, text in enumerate(tqdm(iterable=corpus, desc='Obtaining embeddings', unit='doc')):
            ...
            for sent in text:
                sent = sent[0:510] # Unnecessary for ELMo, but important to compare with BERT.
                sentence_embeddings = self.model.embed_sentence(sent)
                tokens_embeddings = [
                    torch.cat(
                        [
                            torch.FloatTensor(sentence_embeddings[i, token_idx, :])
                            for i in range(len(sentence_embeddings))
                        ], dim=0
                    ).numpy()
                    for token_idx in range(len(sent))
                ]
                sentences_embeddings.append(np.average(tokens_embeddings, axis=0))
...
def train(excel_file, text_column, labels_column, train_test_idxs_file, n_jobs, n_accepted_probs, output_file):
    ...
    ft = FeatureExtractor()
    X_train = ft.extract_features(corpus_train, 'X_train_ELMo.pkl', 'X_train_ELMo.dat')
    X_test = ft.extract_features(corpus_test, 'X_test_ELMo.pkl', 'X_test_ELMo.dat')
    clfs = [
