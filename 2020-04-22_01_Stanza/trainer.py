...
class Preprocessor:
    def __init__(self, lang='pt', stanza_dir='./stanza_resources', processors='tokenize,mwt,pos,lemma', use_gpu=False, nltk_stop_words_package='portuguese'):
        stanza.download(lang=lang, dir=stanza_dir, processors=processors)
        self.nlp = stanza.Pipeline(lang=lang, dir=stanza_dir, processors=processors, use_gpu=use_gpu)
        nltk.download(info_or_id='stopwords', quiet=False)
        self.stop_words = nltk.corpus.stopwords.words(nltk_stop_words_package)

    def preprocess(self, corpus, stanza_docs_file='stanza_docs.pkl'):
        f = open(stanza_docs_file, 'wb')
        pickle.dump({'total': len(corpus)}, f)
        new_corpus = []
        for text in tqdm(corpus, desc='Preprocessing', unit='doc'):
            ...
            tokens = [
                ...
                if word.upos != 'PUNCT' and not word.lemma.lower() in self.stop_words
            ]
            new_corpus.append(tokens)
        f.close()
        new_corpus_str = [' '.join(token_list) for token_list in tqdm(iterable=new_corpus, desc='Joining tokens', unit='doc')]
        return new_corpus, new_corpus_str
