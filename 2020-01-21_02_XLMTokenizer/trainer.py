...
class Preprocessor:
    def __init__(self, model='xlm-mlm-100-1280', nltk_stop_words_package='portuguese'):
        self.tokenizer = XLMTokenizer.from_pretrained(model, cache_dir=model)
    ...
    def preprocess(self, corpus):
        ...
        corpus = [[t for t in token_list if not t.replace('</w>', '') in punctuation] for token_list in tqdm(corpus, desc='Removing punctuation', unit='doc')]
        corpus = [[t for t in token_list if not t.replace('</w>', '') in self.stop_words] for token_list in tqdm(corpus, desc='Removing stop words', unit='doc')]
